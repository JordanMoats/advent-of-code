import { day12 } from "../input";import { day12 } from "../input";
...
function getPossibleArrangements(str: string, nums: number[]): string[] {
    ...
    const possibleCombinations = getSubsetsOfSizeN(
        qMarkIndices,
        hashGoal - hashCount
    );
...
function run() {
    ...
    const possibleArrangements: string[][] = lines.map((line) => {
        let [str, numsString] = line.split(" ");
        const nums = numsString.split(",").map((num) => parseInt(num));
        let numsTimes5: number[] = nums;
        for (let i = 0; i < 4; i++) {
            str += "?" + str;
            numsTimes5 = [...numsTimes5, ...nums]; 
        }
        return getPossibleArrangements(str, numsTimes5);
    });
